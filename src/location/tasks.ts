...
}>(TASK, async ({ data, error, executionInfo }) => {
  if (error || !data?.region?.identifier || !executionInfo.eventId) {
    await write("safe_error", "TASK_ERROR").catch(() => {});
    return;
  }
  const kind =
  ...
  try {
    ...
    if (
      await capture(
        ...
        !observedThisRuntime.has(data.region.identifier),
      )
    ) {
      observedThisRuntime.add(data.region.identifier);
      await sync();
    }
  } catch {
    await write("safe_error", "STORAGE_ERROR").catch(() => {});
  }
});
