...
export async function permissions() {
  const [foreground, background, enabled, registered] = await Promise.all([
    Location.getForegroundPermissionsAsync(),
    Location.getBackgroundPermissionsAsync(),
    Location.hasServicesEnabledAsync(),
    Location.hasStartedGeofencingAsync(TASK),
  ]);
  return {
...
export async function pause(reason: Closure["reason"] = "paused") {
  ...
  try {
    return await pauseLocal(reason);
  } finally {
...
export async function resume() {
  ...
  if (p.foreground !== "granted" || p.background !== "granted" || !p.enabled)
    throw new Error("PERMISSION_REQUIRED");
  const epoch = await pause();
  await sync();
  const owner = await authenticatedOwner();
  const setup = await rpc("get_my_setup_state", {});
  ...
  if (!installation) {
    ...
    await write("installation", installation);
  }
  const device = await rpc("claim_device", {
    installation_id: installation,
    version_info: { app: "1.0.0", os: String(Platform.Version) },
  });
  ...
  const regions = places.map((place) => ({
    identifier: `${session.id}:${place.id}`,
    place_id: place.id,
  ...
  try {
    await transaction(async (db) => {
      ...
      if (current.owner !== owner || (await read("epoch", 0, db)) !== epoch)
        throw new Error("AUTH_REQUIRED");
      await write(
        "state",
        { owner, paused: false, session, device: device.id, regions },
        db,
      );
    });
    await Location.startGeofencingAsync(
      TASK,
      regions.map((r) => ({ ...r, notifyOnEnter: true, notifyOnExit: true })),
    );
  } catch {
    if ((await state()).session?.id === session.id) await pause();
    await rpc("end_capture", {
  ...
  if (current.paused || current.session?.id !== session.id)
    await Location.stopGeofencingAsync(TASK);
}
export async function reconcile() {
  ...
  if (current.paused || !current.session) {
    ...
    p.foreground !== "granted" ||
    p.background !== "granted" ||
    !p.enabled ||
    !p.registered
  ) {
    await pause();
    await write("safe_error", "PERMISSION_REQUIRED");
  } else {
    const task = (await TaskManager.getRegisteredTasksAsync()).find(
      (t) => t.taskName === TASK,
    );
    const actual = (
      task?.options?.regions as Location.LocationRegion[] | undefined
    )
      ?.map((r) => r.identifier)
      .sort();
    if (
      JSON.stringify(actual) !==
      JSON.stringify(current.regions.map((r) => r.identifier).sort())
    ) {
      await pause();
      await write("safe_error", "REGISTRATION_ERROR");
      await sync();
      return;
    }
    try {
      ...
      /* Offline monitoring stays active; sync reports auth/network state. */
    }
  }
  await sync();
...
export async function registeredRegions() {
  return (
    (await TaskManager.getRegisteredTasksAsync()).find(
      (t) => t.taskName === TASK,
    )?.options ?? null
  );
}
