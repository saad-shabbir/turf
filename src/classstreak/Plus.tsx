...
export function Plus({close,start}:{close:()=>void;start:()=>void}){
 ...
 return <Screen style={{gap:10,padding:20}} footer={<><Button title="Start 14 days free" onPress={start}/><Txt muted size={11} style={{textAlign:"center"}}>Then $29.99 a year. That’s $2.50 a month. Cancel any time.</Txt></>}>
  <Row style={{justifyContent:"flex-end"}}><Pressable accessibilityRole="button" accessibilityLabel="Close ClassStreak Plus" onPress={close} style={{width:44,height:44,borderRadius:30,backgroundColor:t.card,borderWidth:1,borderColor:t.line,justifyContent:"center",alignItems:"center"}}><Icon name="x" size={19}/></Pressable></Row>
  <Txt bold size={12} style={{color:t.accent}}>ClassStreak+</Txt><Txt serif size={34}>See yourself better.</Txt><Txt muted size={14} style={{marginBottom:12}}>Everything free stays free. This adds the stuff you’d screenshot.</Txt>
