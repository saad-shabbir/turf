...
export function Scanner({action}:{action:Action}){
 ...
 return <View style={{gap:14}}><Txt serif size={32}>Scan a code</Txt><CameraView style={{height:360,borderRadius:22}} barcodeScannerSettings={{barcodeTypes:["qr"]}} onBarcodeScanned={scanned?undefined:e=>{const studio=e.data.match(/\/s\/([0-9a-f-]{36})(?:[/?#]|$)/i);if(studio){setScanned(true);action("studio_link",{code:studio[1]});return;}const match=e.data.match(/\/j\/([A-Z0-9]{12})(?:[/?#]|$)/i);if(match){setScanned(true);action("social",{action:"invite",payload:{code:match[1]}});action("navigate",{pane:"friends"});}else action("message",{text:"This is not a ClassStreak friend code."});}}/><Button secondary title="Enter a code instead" onPress={()=>action("navigate",{pane:"add-friends"})}/></View>;
}
