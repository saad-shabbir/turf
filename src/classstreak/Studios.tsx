...
export type StudioData={venue_id:string;name:string;visits:number;next_milestone:number;regulars:number;sample_visits:boolean;board:{row_id:string;name:string;weekly_count:number;is_me:boolean}[]};
export function StudioView({snapshot:s,data,selected,onSelect,action}:{snapshot:Snapshot;data:StudioData|null;selected:string;onSelect:(id:string)=>void;action:Action}){
 ...
 <Row><Txt serif size={24} style={{flex:1}}>Regulars this week</Txt><Txt muted size={12}>Mon–Sun</Txt></Row>
 {data&&!data.board.length&&<Card><Txt muted>No one is on this week’s board yet.</Txt></Card>}
