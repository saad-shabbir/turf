...
export type ActivityKey = (typeof activities)[number]["key"] | `custom:${string}`;
...
export type WeekDetail = {day:number;activity_key:ActivityKey;workout_label:string;time_of_day:TimeOfDay;duration_sec:number};
export type Friend = { week_details?:WeekDetail[]; id: string; first_name: string; status: "pending" | "accepted"; requested_by: string; weekly_count: number; weekly_goal: number; streak: number; days: number[]; usual_days: number[]; is_demo: boolean; time_of_day?: TimeOfDay; nudge_available?: boolean };
...
export type Snapshot = { achievements?:{id:string;user_id:string;first_name:string;count:number;source:Source}[]; profile: Profile; goals: Goal[]; pending_goals: Goal[]; usual_days: { activity_key?:ActivityKey; weekday: number; time_of_day: TimeOfDay }[]; places: Place[]; sessions: Session[]; weeks: Week[]; friends: Friend[]; feed: FeedItem[]; inbox: InboxItem[]; server_time: string };
