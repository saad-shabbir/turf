...
import {controlWorkout,onSessionsCreated,reconcileTracking,startTracking,stopTracking,syncVisits,trackingState,trackingNotice,TASK,FIX_TASK} from "./tracking";
...
import {syncPhotos,discardQueuedPhotos} from "./photos";
...
async function applyPendingLinks(){
 const d=await read('cs:draft',newDraft());await getSnapshot();
 if(d.invite_code){await call('cs_social',{action:'invite',payload:{code:d.invite_code}});await write('cs:draft',{...d,invite_code:''});}
 if(d.place&&await read('cs:pending_studio',false)){await saveLinkedPlace(d.place);await write('cs:pending_studio',false);}
}
...
export default function ClassStreakApp() {
  ...
  useEffect(() => {
    ...
    const handle = async (url: string) => {
      ...
      if (hash.get("type") === "recovery" && hash.get("access_token") && hash.get("refresh_token")) {
        ...
        if(auth.session){try{await saveLinkedPlace(place);await refresh();setPane('studios');}catch(e){if(e instanceof Error&&e.message==='SETUP_REQUIRED'){change({place});await write('cs:pending_studio',true);}else throw e;}}else{change({place});await write("cs:pending_studio",true);}
      }
    };
  ...
  const action:Action=(name,payload={})=>{
    ...
    run(async()=>{
      ...
      else if(name==="studio_link"){const {data,error}=await backend().rpc("cs_studio_preview",{code:payload.code});if(error||!data)throw new Error("That studio link is unavailable.");setSnapshot(await saveLinkedPlace({...data,id:undefined}));setPane("studios");}
  ...
  return <Theme name={snapshot?.profile.theme ?? draft.theme}><SafeAreaView style={{ flex: 1,backgroundColor:themes[snapshot?.profile.theme??draft.theme].paper }} edges={["top", "bottom"]}>
    ...
    {!ready || !fontsLoaded ? <Screen><Logo /><Txt muted>Opening your streak…</Txt></Screen> : pane==="privacy"&&!snapshot?<Screen>{extra("privacy")}<Button title="Back" onPress={()=>setPane("home")}/></Screen>:mode !== "onboarding" ? <Screen><Logo /><Txt serif size={35}>{mode === "signin" ? "Welcome back." : mode === "forgot" ? "Forgot password?" : "Choose a new password."}</Txt>
      ...
      <Button title={busy ? "Please wait…" : mode === "signin" ? "Sign in" : mode === "forgot" ? "Send reset link" : "Save password"} disabled={busy} onPress={() => run(async () => {
        if (mode === "signin") { await signIn(email, password); setPassword("");await applyPendingLinks().catch(e=>{if(!(e instanceof Error&&e.message==='SETUP_REQUIRED'))setMessage(safeMessage(e));}); await refresh(); setMode("onboarding"); }
    ...
    </Screen> : snapshot ? pane==="plus"?<Plus close={()=>setPane("profile")} start={()=>{setMessage("Coming soon");setPane("profile");}}/>:(pane==="post"||pane.startsWith("post:"))?<Post snapshot={snapshot} sessionId={pane.startsWith("post:")?pane.slice(5):undefined} action={action} now={new Date(observedNow+clockOffset)} onSaved={refresh}/>:<Product activeWorkout={activeWorkout} busy={busy} snapshot={snapshot} pane={pane} action={action} extra={extra} now={new Date(observedNow+clockOffset)} tracking={tracking}/> : <Onboarding key={draft.step} draft={draft} change={change} next={() => change({ step: Math.min(9, draft.step + 1) })} requestLocation={() => run(async () => {
