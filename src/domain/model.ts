...
export type LocalState = { owner: string | null; paused: boolean; session: Capture | null; device: string | null; regions: Registry[] };
export const emptyState: LocalState = { owner: null, paused: true, session: null, device: null, regions: [] };
export type Setup = { profile: { display_name: string } | null; settings: { collection_consent_at: string | null; share_gym: boolean; share_work: boolean; share_mosque: boolean; display_timezone: string } | null; pair: { id: string; status: 'pending' | 'active'; user_a: string; user_b: string | null } | null; device: { id: string; active: boolean } | null };
export type IngestResult = { accepted: string[]; duplicates: string[]; rejected: { event_id: string; code: string }[]; server_time: string };
export function mayCapture(state: LocalState, identifier: string) {
  return !!(state.owner && !state.paused && state.session && state.device === state.session.device_id && state.regions.some(r => r.identifier === identifier));
}
export function safeError(error: unknown): string {
  const message = error instanceof Error ? error.message : '';
  const codes = ['NOT_ALLOWLISTED','AUTH_REQUIRED','PAIR_REQUIRED','CONSENT_REQUIRED','PLACE_REQUIRED','WRONG_DEVICE','STALE_SESSION','NOT_OWNER','INVALID_INPUT','PLACE_LIMIT','BACKEND_NOT_CONFIGURED','STORAGE_ERROR','REGISTRATION_ERROR','PERMISSION_REQUIRED'];
