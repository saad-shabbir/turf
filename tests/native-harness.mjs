...
globalThis.__turfMocks = {
  ...
  "@supabase/supabase-js": {
    createClient: () => ({
      auth: {
        getSession: async () => ({
          data: { session: native.session },
          error: null,
        }),
        refreshSession: async () => {
          native.refreshes++;
          return { data: { session: null }, error: new Error("expired") };
        },
      },
      rpc: async (name, args) => {
        native.uploads.push({ name, args });
        if (native.networkFails)
          return { data: null, error: { message: "offline" } };
        return {
          data: native.rpcHandler
            ? await native.rpcHandler(name, args)
            : native.result,
          error: null,
        };
      },
      from: () => ({
        select: () => ({
          eq: async () => ({
            data: [{ id: "place-a", latitude: 0, longitude: 0, radius_m: 150 }],
            error: null,
          }),
        }),
      }),
  ...
  "expo-task-manager": {
    defineTask: (name, fn) => native.tasks.set(name, fn),
  ...
  "expo-location": {
    ...
    hasStartedGeofencingAsync: async () => native.registered,
  ...
  "react-native": { Platform: { Version: "unit" } },
};
registerHooks({
  resolve(specifier, context, next) {
    if (Object.hasOwn(globalThis.__turfMocks, specifier))
    ...
    if (
      ...
      return next(pathToFileURL(path + ".ts").href, context);
    }
  ...
  load(url, context, next) {
    if (url.startsWith("mock:")) {
      const name = url.slice(5),
        keys = Object.keys(globalThis.__turfMocks[name]);
      return {
        format: "module",
        shortCircuit: true,
        source: keys
          .map(
            (k) =>
              `export const ${k}=globalThis.__turfMocks[${JSON.stringify(name)}][${JSON.stringify(k)}];`,
          )
          .join("\n"),
      };
    }
