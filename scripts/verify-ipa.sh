...
assert p['CFBundleShortVersionString']=='2.0.0' and p['CFBundleVersion']=='2'
assert p['CFBundleDisplayName']=='ClassStreak'
...
for extension in app.rglob('*.appex'):
 ...
 assert info['CFBundleIdentifier']=='com.turf.privatealpha.widgets','Unexpected extension'
...
if codesign -d --entitlements :- "${app[0]}" > "$temp/entitlements.plist" 2>/dev/null; then
  ! grep -Eq 'aps-environment|com.apple.developer.applesignin|associated-domains' "$temp/entitlements.plist"
fi
